...
history_cnn7 = model_cnn7.fit([matrix_train, matrix_all_train], ccs_train, epochs=100, batch_size=24, validation_data=([matrix_val, matrix_all_val], ccs_val), callbacks=[early_stopping])

history_df_cnn7 = pd.DataFrame(history_cnn7.history)
history_df_cnn7.to_csv('/home/emmy/Notebooks2/history_cnn7.csv', index=False)
print("Minimum validation loss: {}".format(history_df_cnn7['val_loss'].min()))

# Predict CCS values test set
ccs_test["Model_cnn7_predictions"] = model_cnn7.predict((matrix_test, matrix_all_test))

ccs_test.to_csv('/home/emmy/Notebooks2/ccs_test_model_cnn7.csv', index=False)