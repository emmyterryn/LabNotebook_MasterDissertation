...
history_rnn9 = model_rnn9.fit([matrix_train, matrix_all_train], ccs_train, epochs=100, batch_size=256, validation_data=([matrix_val, matrix_all_val], ccs_val), callbacks=[early_stopping])

history_df_rnn9 = pd.DataFrame(history_rnn9.history)
history_df_rnn9.to_csv('/home/emmy/Notebooks2/history_rnn9.csv', index=False)
print("Minimum validation loss: {}".format(history_df_rnn9['val_loss'].min()))

# Predict CCS values test set
ccs_test["Model_rnn9_predictions"] = model_rnn9.predict((matrix_test, matrix_all_test))

ccs_test.to_csv('/home/emmy/Notebooks2/ccs_test_model_rnn9.csv', index=False)